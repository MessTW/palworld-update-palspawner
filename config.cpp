...
bool config::IsInGame(SDK::UObject* world)
{
    ...
    auto pGameState = aPalUtility->GetPalGameStateInGame(world);
    if (!pGameState)
        return false;

    if (pGameState->MaxPlayerNum >= 1)
        return true;
    else
        return false;
}
...
bool config::IsAlive(SDK::APalCharacter* pChar)
{
